...
int al_containsAll(ArrayList* this,ArrayList* this2)
{
    ...
    {
        returnAux=0;
        for(index2=0; index2<this2->size; index2++)
        {
            for(index=0; index<this->size; index++)
                if(this2->pElements[index2]==this->pElements[index])
                    break;
            if(index==this->size)
                break;
        }
        if(index2==this2->size)
            returnAux=1;
    }
...
void quickSortP(void** array,int (*pFunc)(void*,void*), int size, int order)
{
    ...
    {
        while(order==pFunc(pivot,array[i]))
            i++;
        while(order==pFunc(array[j],pivot))
            j--;
        if(i>=j)
            break;
        auxiliar  = array[i];
...
{
    int returnAux = -1;
    int extraResize=AL_INCREMENT;
    if(this!=NULL)
    {
        if(incrementScaleIncrease(this)==1)
            extraResize*=10;
        if(this->size>this->reservedSize)
            returnAux=arrayListRealloc(this,sizeof(this->size));
        else if((this->size+1)==this->reservedSize)
            returnAux=arrayListRealloc(this,sizeof(this->size+extraResize));
        else
            returnAux=0;
    }
    return returnAux;
